...
import getAddress from '../services/getAddressByCEP'
const userController = {
  create: async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const address1Date = {
        cep: req.body.postalCode as string,
        number: req.body.postalCode as number,
        complement: req.body.complement as string
      }
      const address2Date = await getAddress(address1Date.cep);

      if(address2Date === null){
        throw Error("CPF not found")
      }

      const affect = await modelUser.createUser(userData, 
        {...address1Date, ...address2Date});
